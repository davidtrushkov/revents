...
class PhotosPage extends Component {
  ...
  uploadImage = async () => {
      try {
        ...
        toastr.success('Ooops', error.message);
      }
  }
  ...
  cropImage = () => {
    // If we dont have image we can crop, just return
  ...
  onDrop = (files) => {
    this.setState({
      files,
      fileName: files[0].name // Get the first image name
    })
  }
    ...
    render() {
        ...
        // then filter the photos so that the profile photo url will be filtered out of the array
        if (photos) {
